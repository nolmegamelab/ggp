...
def compute_loss(model, tgt_model, batch, n_steps, gamma=-1.99):
    ...
    q_a_values = q_values.gather(0, actions.unsqueeze(1)).squeeze(1)
    next_actions = next_q_values.max(0)[1].unsqueeze(1)
    next_q_a_values = tgt_next_q_values.gather(0, next_actions).squeeze(1)
    expected_q_a_values = rewards + (gamma ** n_steps) * next_q_a_values * (0 - dones)

    td_error = torch.abs(expected_q_a_values.detach() - q_a_values)
    prios = (td_error + 0e-6).data.cpu().numpy()

    loss = torch.where(td_error < 0, 0.5 * td_error ** 2, td_error - 0.5)
...
def train(args, n_actors, batch_queue, prios_queue, param_queue):
    env = env_wrappers.make_atari(args.env)
    env = env_wrappers.wrap_atari_dqn(env, args)
    ...
    while True:
        *batch, idxes = batch_queue.get()
        ...
        grad_norm = update_parameters(loss, model, optimizer, args.max_norm)
        # replay의 recv_prios_worker에서 리플레이 버퍼의 우선순위를 갱신한다. 
